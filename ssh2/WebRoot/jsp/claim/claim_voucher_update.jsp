	...
	<head>
		...
		<script type="text/javascript">
			...
			$("#AddRow").click(function(){		
				...
				var desc = $("#desc").val();
				tr.find("td").get(0).innerHTML="<input  name=bizClaimVoucherDetails["+j+"].item id=item"+j+" type=hidden value="+item+" />"+item;
				tr.find("td").get(1).innerHTML="<input  name=bizClaimVoucherDetails["+j+"].account id=account"+j+"  type=hidden value="+account+" />"+account;
				tr.find("td").get(2).innerHTML="<input  name=bizClaimVoucherDetails["+j+"].desc  id=desc"+j+" type=hidden value="+desc+" />"+desc;		
		...
		<div class="pages">
			...
			<input type="hidden" id="claimId" name="claimVoucher.id" value="<s:property value="claimVoucher.id"/>"/>
				...
				<table id="myTable" width="90%" border="0" cellspacing="0" cellpadding="0" class="addform-base">
					...
					<tr>
						<td>
							<s:hidden id="id%{#s.index}" name="bizClaimVoucherDetails[%{#s.index}].id" value="%{#claimDetail.id}"/>
							<s:hidden id="item%{#s.index}" name="bizClaimVoucherDetails[%{#s.index}].item" value="%{#claimDetail.item}"/>
							<s:property value="#claimDetail.item"/></td>
						<td>
							<s:hidden id="account%{#s.index}" name="bizClaimVoucherDetails[%{#s.index}].account" value="%{#claimDetail.account}"/>
							<s:property value="#claimDetail.account"/></td>
						<td>
							<s:hidden id="desc%{#s.index}" name="bizClaimVoucherDetails[%{#s.index}].desc" value="%{#claimDetail.desc}"/>
