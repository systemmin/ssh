				...
				.click(
						function() {
							...
							if (account != "" && desc != "") {
								...
								tr.find("td").get(0).innerHTML = "<input  name=bizClaimVoucherDetails["+j+"].item id=item"+j+" type=hidden value="+item+" />"
										+ item;
								tr.find("td").get(1).innerHTML = "<input  name=bizClaimVoucherDetails["+j+"].account id=account"+j+"  type=hidden value="+account+" />"
										+ account;
								tr.find("td").get(2).innerHTML = "<input  name=bizClaimVoucherDetails["+j+"].desc  id=desc"+j+" type=hidden value="+desc+" />"
										+ desc;
								...
								$("#account").next(".notice").text("*");
								$("#desc").next(".notice").text("*");
								$(".notice").text("*");
							} else {
								sa--;
								check();
		...
		<div class="pages">
			<s:form action="claimVoucher_saveClaimVoucher.action"
				...
				<table width="90%" border="0" cellspacing="0" cellpadding="0"
					...
					<tr>
						<td>*填报人：</td>
						<td><s:property value="#session.employee.name" />
						</td>
						<td>*填报时间：</td>
						<td>
							<%
								Date date = new Date();
									SimpleDateFormat d = new SimpleDateFormat("YYYY-MM-DD HH:ss");
									String str = d.format(date);
							%><%=str%></td>
					</tr>
					<tr>
						<td>*总金额：￥</td>
						<td><input type="number" id="totalAccount"
							name="claimVoucher.totalAccount" value="" readonly="readonly" />
						</td>
						<td>*状态：</td>
						<td><input type="text" id="status" name="claimVoucher.status"
							value="新创建" readonly="readonly" />
						</td>
					</tr>
					<tr>
						<td colspan="4"><span class="notice" id="noctice">*</span>
						</td>
					</tr>
				</table>
				...
				<table id="detailTable" width="90%" border="0" cellspacing="0"
					...
					<tr>
						...
						<td width="30%"><input type="text"
							name="claimVoucherDetail.desc" id="desc" /><span class=notice>*</span>
						</td>
				...
				<table>
					<tr>
						<td>*事由：</td>
						<td colspan="3"><textarea rows="5" cols="66"
								name="claimVoucher.event" id="event"></textarea></td>
					</tr>
					<tr align="center" colspan="4">
						<td>&nbsp;</td>
						<td><input type="button" id="sub" name="sub" value="保存" 	class="submit_01" /> 
						<input type="submit" id="" name="name="sub" value="提交" class="submit_01" />
						<!--onclick="submitClaimVoucher('提交')"   -->
							<!-- onclick="submitClaimVoucher('保存')" --></td>
					</tr>
				</table>
			</s:form>
		</div>
